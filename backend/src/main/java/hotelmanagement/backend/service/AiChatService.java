    ...
    public String generateChatResponse(String employeeQuestion) {
        try {
            ...
            String systemPromptContext = String.format(
                    ...
                    "- Khi khách/nhân viên hỏi tra cứu, tìm kiếm hoặc kiểm tra thông tin liên quan đến đơn đặt phòng (booking) hoặc hóa đơn (invoice) của một khách hàng cụ thể (ví dụ: tìm theo tên, số điện thoại, mã đặt phòng, mã hóa đơn), bạn phải sử dụng công cụ/hàm `checkBookingAndInvoiceFunction` được cung cấp.\n" +
            ...
            Prompt prompt = new Prompt(
                    ...
                    GoogleGenAiChatOptions.builder()
                            .toolNames(Set.of("checkRoomAvailabilityFunction", "checkBookedRoomsFunction", "checkBookingAndInvoiceFunction"))
                            .build()
            );
