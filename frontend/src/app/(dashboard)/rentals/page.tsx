...
//
//   // Chuẩn hóa trạng thái để phân nhóm bộ lọc
...
//         alert("Đã hủy phiếu thuê phòng thành công!");
//       } catch (err: any) {
//         alert(err.message || "Không thể hủy phiếu thuê phòng");
//       }
//     }
//   };
//
...
//             <option value="Đang sử dụng">Đang sử dụng (Checked-in)</option>
...
export default function RentalsPage() {
  ...
  const handleCheckOutRental = async (bookingId: number, rentalCode: string) => {
    ...
    try {
      ...
      await checkOutBooking(bookingId, "Tiền mặt");
      alert("Trả phòng thành công! Phiếu đã được cập nhật sang trạng thái 'Đã trả phòng' và tạo dữ liệu hóa đơn.");

      // Reload lại trang để cập nhật trạng thái mới hiển thị lên giao diện bảng
