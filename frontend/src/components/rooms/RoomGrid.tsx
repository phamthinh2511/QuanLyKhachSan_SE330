...
const roomsByFloor = useMemo(() => {
  ...
  Object.keys(groups).forEach((floor) => {
    groups[Number(floor)].sort((a, b) => a.roomNumber.localeCompare(b.roomNumber));
  });

  return Object.entries(groups).sort(([floorA], [floorB]) => Number(floorA) - Number(floorB));
}, [rooms]);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            {floorRooms.map((r) => (
              <div 
                key={r.id} 
                className={clsx(
                  "bg-white border border-gray-100 border-l-4 rounded-2xl p-5 shadow-sm hover:shadow-lg hover:-translate-y-1.5 hover:translate-x-1.5 transition-all duration-300 flex flex-col gap-3", 
                  borderStyle[r.status]
                )}
              >
